...
class GameMenuMedium extends StatefulWidget {
  final double fieldSize = 40;
  final Color borderColor = Colors.grey;

  @override
  State<StatefulWidget> createState() => _GameMenuMediumState();
}

class _GameMenuMediumState extends State<GameMenuMedium> {
...
_createtimer();
 ...
 _unlightano(){
  ...
  void _settimer() async{
    SharedPreferences prefs= await SharedPreferences.getInstance();
      if(prefs.containsKey('mhr')){

        setState(() {
          min=prefs.getInt('mmin');
          hr=prefs.getInt('mhr');
          sec=prefs.getInt('msec');
        });
  ...
  _savetimer()async{
    ...
    SharedPreferences prefs= await SharedPreferences.getInstance();
    prefs.setInt('mhr', hr);
    prefs.setInt('mmin', min);
    prefs.setInt('msec', sec);
  }
